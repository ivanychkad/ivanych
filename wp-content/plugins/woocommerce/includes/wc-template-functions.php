...
function ingredients() {
    echo get_field( 'ingredients_for_products' );
}

function directions() {
    echo get_field( 'directions_for_products' );
}
...
if ( ! function_exists( 'woocommerce_default_product_tabs' ) ) {
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
        ...
        // Ingredients - shows product content
        // ensure ACF is available
        if ( !function_exists( 'get_field' ) )
            return;

        $content = trim( get_field( 'ingredients_for_products' ) );
        ...
        // Directions - shows product content
        // ensure ACF is available
        if ( !function_exists( 'get_field' ) )
            return;

        $content = trim( get_field( 'directions_for_products' ) );

        if ( !empty( $content ) ) {

            $tabs['directions'] = array(
                'title' => 'Directions',
                'priority' => 15,
                'callback' => 'directions'
            );
