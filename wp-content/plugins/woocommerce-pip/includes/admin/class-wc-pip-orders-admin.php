...
class WC_PIP_Orders_Admin {
	...
	public function display_order_invoice_number( $wc_order ) {
		...
		if ( is_numeric( $wc_order ) ) {
			$wc_order = wc_get_order( (int) $wc_order );
		}
	...
	public function add_order_status_column_content( $column ) {
		...
		if ( $order_id && in_array( $column, array( 'pip_print_invoice', 'order_title' ), true ) ) {
			...
			} elseif ( 'order_title' === $column && $invoice ) {

				$invoice_number = SV_WC_Order_Compatibility::get_meta( $wc_order, '_pip_invoice_number', true );
		...
		if ( $order_id && 'pip_print_packing-list' === $column ) {
		...
		if ( $order_id && 'order_actions' === $column ) {
	...
	private function get_print_status( $document ) {
		return $document instanceof WC_PIP_Document && $document->get_print_status() ? '&#10004' : '<strong>&ndash;</strong>';
	}
