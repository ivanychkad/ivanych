...
class WC_PIP_Document_Invoice extends WC_PIP_Document {
	...
	public function document_header( $type, $action, $document, $order ) {
		...
		$heading .= sprintf( '<h5 class="order-date">' . esc_html__( 'Order Date: %s', 'woocommerce-pip' ) . '</h5>', date_i18n( wc_date_format(), SV_WC_Order_Compatibility::get_date_created( $order )->getTimestamp() ) );
