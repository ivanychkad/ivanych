...
abstract class WC_PIP_Document {
	...
	public function get_invoice_date() {
		...
		$invoice_date = date_i18n( wc_date_format(), $order_datetime->getTimestamp() );
		// For backwards compatibility, we keep the order date as a mysql string as before WC 3.0.
		$order_date   = date( 'Y-m-d H:i:s', $order_datetime->getTimestamp() );
	...
	public function get_items_count() {
		...
		foreach ( $items as $item_id => $item ) {
			...
			// Add the refunded qty, which is a negative integer
	...
	protected function get_order_item_quantity_html( $item_id, $item ) {
		...
		if ( $this->has_refunds ) {
			...
			// We *add* refunded qtys because they're negative integers.
	...
	protected function get_order_item_weight_html( $item_id, $item, $product ) {
		...
		$item_weight    = 0;
		$product_weight = SV_WC_Product_Compatibility::get_prop( $product, 'weight' );
