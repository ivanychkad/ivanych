 ...
 * @return array
 */
function chefplaza_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! chefplaza_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'chefplaza_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'chefplaza_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'chefplaza' ) . $network['name'],
