...
class Chefplaza_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'chefplaza' );
		$this->widget_id          = 'chefplaza_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'chefplaza' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'chefplaza' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'chefplaza' ),
				'label' => esc_html__( 'Subscribe Title', 'chefplaza' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'chefplaza' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'chefplaza' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'chefplaza' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'chefplaza' ),
				'label' => esc_html__( 'Subscribe submit label', 'chefplaza' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'chefplaza' ),
				'label' => esc_html__( 'Subscribe success', 'chefplaza' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'chefplaza' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'chefplaza' ),
				'label' => esc_html__( 'Follow Title', 'chefplaza' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'chefplaza' ),
			),
		);

		add_action( 'wp_ajax_chefplaza_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_chefplaza_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'chefplaza' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'chefplaza' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'chefplaza' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'chefplaza' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'chefplaza' );
		}
	...
	public function get_social_nav() {
		return chefplaza_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = chefplaza_render_icons( $subscribe_submit );

		return '<a href="#" class="subscribe-block__submit btn-primary ' . esc_attr( $class ) . '">' . wp_kses_post(
			$subscribe_submit ) . '</a>';
	}
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'chefplaza_subscribe' ) ) {
...
add_action( 'widgets_init', 'chefplaza_register_subscribe_follow_widgets' );
function chefplaza_register_subscribe_follow_widgets() {
	register_widget( 'Chefplaza_Subscribe_Follow_Widget' );
}
