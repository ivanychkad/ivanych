 ...
 * @package Chefplaza
 */
if ( ! class_exists( 'Chefplaza_About_Author_Widget' ) ) {

	class Chefplaza_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'chefplaza widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'chefplaza' );
			$this->widget_id          = 'chefplaza_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'chefplaza' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'chefplaza' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'chefplaza' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'chefplaza' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'chefplaza' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'chefplaza' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'chefplaza' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'chefplaza' ),
					'label' => esc_html__( 'Link label', 'chefplaza' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'chefplaza' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'chefplaza-author-avatar' );
...
add_action( 'widgets_init', 'chefplaza_register_about_author_widgets' );
function chefplaza_register_about_author_widgets() {
	register_widget( 'Chefplaza_About_Author_Widget' );
}
