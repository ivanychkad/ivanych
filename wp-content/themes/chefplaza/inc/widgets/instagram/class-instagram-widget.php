 ...
 * @package Chefplaza
 */
if ( ! class_exists( 'Chefplaza_Instagram_Widget' ) ) {
	class Chefplaza_Instagram_Widget extends Cherry_Abstract_Widget {
		/**
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'chefplaza' );
			$this->widget_id          = 'chefplaza_widget_instagram';
			$this->widget_name        = esc_html__( 'Instagram', 'chefplaza' );
			$this->settings           = array(
				'title'           => array(
					'type'  => 'text',
					'value' => esc_html__( 'Instagram', 'chefplaza' ),
					'label' => esc_html__( 'Title', 'chefplaza' ),
				),
				'tag'             => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'chefplaza' ),
				),
				'image_counter'   => array(
					'type'       => 'stepper',
					'value'      => '6',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Number of photos', 'chefplaza' ),
				),
				'display_caption' => array(
					...
					'options' => array(
						'display_caption_check' => esc_html__( 'Caption', 'chefplaza' ),
					),
				),
				'display_date'    => array(
					'type'    => 'checkbox',
					...
					'options' => array(
						'display_date_check' => esc_html__( 'Date', 'chefplaza' ),
					),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $instance['tag'] ) ) {
				return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'chefplaza' ) . $args['after_widget'];
			}
			ob_start();
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered an invalid hashtag.',
						'chefplaza' ) . $args['after_widget'];
			}
			$this->widget_start( $args, $instance );
			$template = locate_template( 'inc/widgets/instagram/views/instagram.php', false, false );
			printf( '<div class="%s">', join( ' ', apply_filters( 'chefplaza_instagram_widget_wrapper_class', array( 'instagram__items' ) ) ) );
		...
		public function get_photos( $data, $img_counter, $config ) {
			$transient_key = $this->get_transient_key( array( 'image_counter' => $img_counter, 'tag' => $data ) );
			$cached        = get_transient( $transient_key );
			if ( false !== $cached ) {
				return $cached;
			}
			$url      = add_query_arg( array( '__a' => 1 ), sprintf( $this->get_tags_url(), $data ) );
			 ...
			 * @var string
			 */
			$order = apply_filters( 'chefplaza_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt( $photo['caption'], apply_filters( 'chefplaza_instagram_widget_caption_length', 10 ), apply_filters( 'chefplaza_instagram_widget_caption_more', '&hellip;' ) );
				}
		...
		public function get_image( $photo ) {
			...
			$width  = 200;
			$height = 200;
			$image  = str_replace( '387x387', "{$width}x{$height}", $photo['image'] );
		...
		public function get_tags_url() {
			return apply_filters( 'chefplaza_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		...
		public function get_post_url() {
			return apply_filters( 'chefplaza_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
	...
	add_action( 'widgets_init', 'chefplaza_register_instagram_widget' );
	function chefplaza_register_instagram_widget() {
		register_widget( 'Chefplaza_Instagram_Widget' );
	}
}