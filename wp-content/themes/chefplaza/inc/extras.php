 ...
 * @return string
 */
function chefplaza_set_sidebar_position( $position ) {
	$queried_obj = apply_filters( 'chefplaza_queried_object_id', false );
	...
	$post_position = get_post_meta( $queried_obj, 'chefplaza_sidebar_position', true );
...
add_filter( 'theme_mod_sidebar_position', 'chefplaza_set_sidebar_position' );
 ...
 * @return string
 */
function chefplaza_render_macros( $string ) {

	$macros = apply_filters( 'chefplaza_data_macros', array(
		'/%%year%%/'           => date( 'Y' ),
		'/%%date%%/'           => date( get_option( 'date_format' ) ),
		'/%%site-name%%/'      => '<span class="sitename">' . get_option( 'blogname' ) . '</span>',
		'/%%privacy-policy%%/' => chefplaza_get_privacy_link(),
	) );
 ...
 * @return string
 */
function chefplaza_get_privacy_link() {
 ...
 * @return string
 */
function chefplaza_render_icons( $content ) {
	$icons     = chefplaza_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'chefplaza_render_icons_callback', $content );
}
 ...
 * @return string
 */
function chefplaza_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function chefplaza_get_render_icons_set() {
	return apply_filters( 'chefplaza_render_icons_set', array(
 ...
 * @return string
 */
function chefplaza_render_theme_url( $url ) {
 ...
 */
function chefplaza_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function chefplaza_post_formats_gallery() {
	$size = chefplaza_post_thumbnail_size();
	...
	$images = chefplaza_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images( false );
 ...
 * @return bool
 */
function chefplaza_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, chefplaza_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function chefplaza_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'chefplaza-thumb-536-257',
		'fullwidth'    => 'chefplaza-thumb-1728-830',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', chefplaza_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', chefplaza_theme()->customizer->get_default( 'blog_featured_image' ) );
	$size        = $args[ $size_option ];
	$link_class  = sanitize_html_class( $args['class_prefix'] . $size_option );

	if ( 'default' !== $layout || is_single() || is_sticky() || in_array( $format, array(
			'image',
			'gallery',
			'link',
		) )
