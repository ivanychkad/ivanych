 ...
 */
function chefplaza_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->chefplaza_comment_list_args = $args;
	$comment = $_comment;
 ...
 */
function chefplaza_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->chefplaza_comment_list_args['avatar_size'] ) ) {
		$size = $comment->chefplaza_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'chefplaza_comment_author_avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function chefplaza_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'chefplaza_get_comment_author_link', sprintf( '<b class="fn">%s</b>', get_comment_author_link() ), $args );
}
 ...
 */
function chefplaza_get_comment_date( $args = array() ) {
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'chefplaza_get_comment_date', sprintf( '<a href="%3$s" class="comment-date"><time datetime="%1$s" class="comment-date__time">%2$s</time></a>', get_comment_time( 'c' ), get_comment_date( $format ), get_comment_link() ), $args );
}
 ...
 */
function chefplaza_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'chefplaza_get_comment_text', $comment_text, $args );
}
 ...
 */
function chefplaza_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'chefplaza_get_comment_reply_link', $reply, $args );
}
 ...
 */
function chefplaza_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', 'chefplaza' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'chefplaza_get_comment_link_edit', $link, $comment->comment_ID, $args );
}
